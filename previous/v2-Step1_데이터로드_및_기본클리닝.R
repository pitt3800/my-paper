...
fever_including_cleaned <- fever_including %>%
  mutate(
    # 텍스트 필드 빈칸 → "Unknown"
    across(c(chief_complaint_1, chief_complaint_2, diagnosis, 
    ...
    # 병원 관련 정보
    across(c(referred_hospital_name, referred_hospital_type),
           ~if_else(is.na(.) | str_trim(as.character(.)) == "", 
                    "None", 
                    as.character(.))),
...
#-----------------------------------------------------------------------------
# 4. Original Cleaned 버전 저장-cleaned 는 변수통일 & 결측치 정리 버전
#------------------------------------------------------------------------------
