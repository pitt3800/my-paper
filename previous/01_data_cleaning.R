...
base <- base %>%
  mutate(
    ...
    discharge_status_clean = case_when(
      ...
      TRUE ~ discharge_status
    ),
    
    # 사망 여부
    death = if_else(
      !is.na(death_date) | discharge_status_clean == "Death", 
      1, 0
    )
  )
...
fever_lab_wide <- fever_lab %>%
  group_by(patient_id, visit_date, order_name) %>%
  summarize(result_numeric = mean(result_numeric, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from = order_name,
    values_from = result_numeric,
    names_prefix = "lab_"
  ) %>%
  clean_names()

cat("Fever Lab wide format 변환 완료\n")
...
cat("   ├── base_ct_lab_clean.{rds,csv}  ⭐ 주요 분석용\n")
