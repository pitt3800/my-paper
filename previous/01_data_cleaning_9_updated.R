...
# =============================================================================
# STEP 2.5: 원본 파일 결측치 사전 처리 (Cleaning)
# =============================================================================

cat("\n=== STEP 2.5: 결측치 사전 처리 ===\n")

# 2.5.1 Base Result - 질환 관련 빈칸 처리
base <- base %>%
  mutate(
    # 질환 관련 변수: 빈칸 → "-" (질환 없음)
    across(c(liver_disease, hypertension, diabetes, heart_disease, 
           ...
           ~if_else(is.na(.) | str_trim(.) == "", "-", .)),
    
    # discharge_status 빈칸 → 'EM discharge'
    discharge_status = if_else(
      is.na(discharge_status) | str_trim(discharge_status) == "",
      "EM discharge",
      discharge_status
    )
  )
...
cat(sprintf("  - 질환 관련 변수 빈칸 → '-' 처리\n"))
cat(sprintf("  - discharge_status 빈칸 → 'EM discharge' 처리\n"))

# 2.5.2 Nurse - 특별한 결측치 처리 없음 (텍스트 필드이므로)
nurse <- nurse %>%
  mutate(
    across(c(procedure, special_note, note),
           ~if_else(is.na(.), "Not recorded", .))
  )

cat("Nurse 결측치 처리 완료\n")

# 2.5.3 Fever Lab - 결측치는 그대로 유지 (검사 안 함을 의미)
cat("Fever Lab 결측치 유지 (검사 미시행 의미)\n")

# 2.5.4 CT - 결측치는 그대로 유지
cat("CT 결측치 유지\n")

# 2.5.5 Cleaned 파일 저장 (원본 구조 유지)
dir.create("cleaned_data/original_cleaned", showWarnings = FALSE, recursive = TRUE)

# Base Result Cleaned
base_cleaned_original <- base_raw %>%
  rename(
    patient_id = 등록번호,
    visit_date = 내원일자,
    visit_time = 내원시간,
...
write_csv(base_cleaned_original, "cleaned_data/original_cleaned/base_result_cleaned.csv")
write_xlsx(base_cleaned_original, "cleaned_data/original_cleaned/base_result_cleaned.xlsx")

# Nurse Cleaned
nurse_cleaned_original <- nurse_raw %>%
  rename(
    patient_id = 등록번호,
    patient_name = 환자명,
    visit_date = 내원일,
  ...
  mutate(
    across(c(procedure, special_note, note),
           ~if_else(is.na(.), "Not recorded", as.character(.)))
  )

write_csv(nurse_cleaned_original, "cleaned_data/original_cleaned/nurse_cleaned.csv")
write_xlsx(nurse_cleaned_original, "cleaned_data/original_cleaned/nurse_cleaned.xlsx")

# Fever Lab Cleaned (결측치는 유지, 단순 rename만)
fever_lab_cleaned_original <- fever_lab_raw %>%
  rename(
    patient_id = 등록번호,
    visit_date = 내원일자,
    visit_time = 내원시간,
...
write_csv(fever_lab_cleaned_original, "cleaned_data/original_cleaned/fever_lab_cleaned.csv")
write_xlsx(fever_lab_cleaned_original, "cleaned_data/original_cleaned/fever_lab_cleaned.xlsx")

# CT Cleaned (결측치는 유지, 단순 rename만)
ct_cleaned_original <- ct_raw %>%
  rename(
    patient_id = 등록번호,
    visit_date = 내원일자,
    visit_time = 내원시간,
...
write_csv(ct_cleaned_original, "cleaned_data/original_cleaned/ct_cleaned.csv")
write_xlsx(ct_cleaned_original, "cleaned_data/original_cleaned/ct_cleaned.xlsx")

cat("\n원본 파일 Cleaned 버전 저장 완료:\n")
...
base <- base %>%
  mutate(
    ...
    # 퇴원 상태 표준화
    # STEP 2.5에서 빈칸은 이미 "EM discharge"로 처리됨
    discharge_status_clean = case_when(
      str_detect(tolower(discharge_status), "사망|death") ~ "Death",
      str_detect(tolower(discharge_status), "em discharge") ~ "ER discharge",
...
base_final <- base_final %>%
  mutate(
    ...
    los_days = if_else(los_days < 0, 0, los_days),  # 음수 → 0
    los_group = case_when(
      is.na(los_days) ~ "<1 week",  # NA 방지
      los_days < 7 ~ "<1 week",
...
base_ct <- base_ct %>%
  mutate(
    ...
    ct_any_positive = if_else(is.na(ct_any_positive), -1, ct_any_positive),  # -1 = CT 안함
...
# Lab 변수의 NA는 "검사 안함"을 의미하므로 중앙값 또는 -1로 대체
# 분석 시 문제가 될 수 있으므로, 분석용 데이터셋에서는 제외하는 것이 좋음
# 하지만 NA를 제거하기 위해 -999로 표시 (검사 안함)
if(length(lab_vars) > 0) {
  base_ct_lab <- base_ct_lab %>%
    mutate(across(all_of(lab_vars), ~if_else(is.na(.), -999, .)))
  
  cat(sprintf("Lab 변수 결측값 → -999 (검사 안함) 처리 완료\n"))
}
...
# 최종 결측값 확인
cat("\n최종 결측값 확인:\n")
cat(sprintf("  base_final: %d개\n", sum(is.na(base_final))))
cat(sprintf("  base_ct: %d개\n", sum(is.na(base_ct))))
cat(sprintf("  base_ct_lab: %d개\n", sum(is.na(base_ct_lab))))
cat(sprintf("  base_full: %d개\n", sum(is.na(base_full))))
...
# 분석용 데이터셋 (텍스트/참고용 필드 및 Lab 변수 제외)
# Lab 변수는 많은 환자가 검사를 하지 않아 -999로 표시됨
# Lab 분석이 필요한 경우 base_ct_lab을 사용하되, -999를 제외하고 분석
...
# Lab이 있는 환자만 포함한 분석용 데이터셋
# wbc, crp 등 주요 검사가 있는 환자만
if("wbc" %in% names(base_ct_lab) && "crp" %in% names(base_ct_lab)) {
  base_with_lab <- base_ct_lab %>%
    filter(wbc != -999 & crp != -999) %>%  # 주요 검사가 있는 환자만
    select(-ends_with("_desc"),
           -admission_diagnosis, -discharge_diagnosis,
           -ct_findings_combined)
