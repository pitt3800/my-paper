# =============================================================================
# Step 3b: 분석된 CT 데이터 통합 및 최종 코호트 생성
# =============================================================================
# 연구: 85세 이상 발열 환자 CT 진단 가치 연구
# 목적: 외부 분석 CT 데이터 통합, 코호트 선정, 최종 데이터셋 생성
# 전제: ct_for_external_analysis.csv 파일이 준비되어 있어야 함
# 예상 소요: 5-7분
# =============================================================================
...
cat("\n")
cat("╔═══════════════════════════════════════════════════════════╗\n")
cat("  Step 3b: 코호트 선정 및 최종 데이터셋 (CT 분석 통합)      \n")
cat("╚═══════════════════════════════════════════════════════════╝\n\n")

#------------------------------------------------------------------------------
# 1. 외부 분석 완료된 CT 데이터 불러오기 및 검증
#------------------------------------------------------------------------------
cat("=== STEP 1: CT 분석 데이터 불러오기 ===\n")

# Part 2의 원본 CT 데이터 로드
ct_original <- readRDS("cleaned_data/part2_ct_summary.rds")
cat(sprintf("✓ 원본 CT 데이터: %d건\n", nrow(ct_original)))
...
ct_analysis_only <- read_csv("ct_for_external_analysis.csv", 
                             ...
                             col_types = cols(
                               ...
                               visit_date = col_skip(),           # 건너뛰기
                               n_ct_scans = col_skip(),           # 건너뛰기
                               ct_findings_combined = col_skip(), # 건너뛰기
  ...
  mutate(
    ...
    fever_focus = case_when(
      is.na(fever_focus) | fever_focus == "" ~ NA_integer_,
    ...
    # disease 결측값 처리
    disease_1 = if_else(is.na(disease_1) | str_trim(disease_1) == "", 
                        "", str_trim(disease_1)),
    disease_2 = if_else(is.na(disease_2) | str_trim(disease_2) == "", 
                        "", str_trim(disease_2)),
    disease_3 = if_else(is.na(disease_3) | str_trim(disease_3) == "", 
                        "", str_trim(disease_3))
  )

cat(sprintf("✓ CSV 분석 결과 로드: %d건\n", nrow(ct_analysis_only)))

# 행 개수 확인
if (nrow(ct_original) != nrow(ct_analysis_only)) {
  cat(sprintf("⚠️  경고: 행 개수 불일치!\n"))
  cat(sprintf("   원본: %d건, CSV: %d건\n", nrow(ct_original), nrow(ct_analysis_only)))
  stop("행 개수가 다릅니다!")
}

# 행 번호로 병합
ct_analyzed <- ct_original %>%
  mutate(row_num = row_number()) %>%
  left_join(
    ct_analysis_only %>% select(row_num, fever_focus, disease_1, disease_2, disease_3),
    by = "row_num"
  ) %>%
  select(-row_num)
...
# 2. 데이터 품질 검증
#------------------------------------------------------------------------------
cat("=== STEP 2: 데이터 품질 검증 ===\n")

# 2.1 필수 열 확인
required_cols <- c("patient_id", "visit_date", "fever_focus", "disease_1")
missing_cols <- setdiff(required_cols, names(ct_analyzed))

if (length(missing_cols) > 0) {
  stop(paste0("❌ 필수 열이 없습니다: ", paste(missing_cols, collapse = ", ")))
}
cat("✓ 필수 열 확인 완료\n")

# 2.2 fever_focus 검증
invalid_fever_focus <- ct_analyzed %>%
  filter(!is.na(fever_focus) & !fever_focus %in% c(0, 1))

if (nrow(invalid_fever_focus) > 0) {
  cat(sprintf("⚠️  경고: fever_focus에 잘못된 값이 있습니다 (%d건)\n", 
              nrow(invalid_fever_focus)))
  cat("   올바른 값: 0 또는 1만 가능\n")
  print(invalid_fever_focus %>% select(patient_id, fever_focus))
  stop("fever_focus 값을 수정 후 다시 실행하세요.")
}

missing_fever_focus <- sum(is.na(ct_analyzed$fever_focus))
if (missing_fever_focus > 0) {
  cat(sprintf("⚠️  경고: fever_focus 결측: %d건 (%.1f%%)\n",
              missing_fever_focus,
              missing_fever_focus / nrow(ct_analyzed) * 100))
}
cat("✓ fever_focus 검증 완료\n")

# 2.3 disease_1 검증
missing_disease1 <- sum(is.na(ct_analyzed$disease_1) | 
                          str_trim(ct_analyzed$disease_1) == "")

if (missing_disease1 > 0) {
  cat(sprintf("⚠️  참고: disease_1 결측: %d건 (%.1f%%)\n",
              missing_disease1,
              missing_disease1 / nrow(ct_analyzed) * 100))
}
cat("✓ disease_1 검증 완료\n\n")

# 2.4 환자 ID 및 날짜 검증
if (any(is.na(ct_analyzed$patient_id))) {
  stop("❌ patient_id에 결측값이 있습니다!")
}
if (any(is.na(ct_analyzed$visit_date))) {
  stop("❌ visit_date에 결측값이 있습니다!")
}
cat("✓ 환자 ID 및 날짜 검증 완료\n\n")

#------------------------------------------------------------------------------
# 3. 데이터 형식 변환 및 정리
#------------------------------------------------------------------------------
cat("=== STEP 3: 데이터 형식 변환 ===\n")

ct_summary_dedup <- ct_analyzed %>%
  mutate(
    # patient_id: character 유지 (다른 데이터셋과 통일)
    patient_id = as.character(patient_id),
    
    # visit_date: Date 형식
    visit_date = as.Date(visit_date),
    
    # fever_focus: integer (0/1), 결측 → 0 처리
    fever_focus = as.integer(fever_focus),
    fever_focus = if_else(is.na(fever_focus), 0L, fever_focus),
    
    # disease 변수들: character, 결측 → "None" 처리
    disease_1 = if_else(is.na(disease_1) | str_trim(disease_1) == "", 
                        "None", str_trim(disease_1)),
    disease_2 = if_else(is.na(disease_2) | str_trim(disease_2) == "", 
                        "None", str_trim(disease_2)),
    disease_3 = if_else(is.na(disease_3) | str_trim(disease_3) == "", 
                        "None", str_trim(disease_3)),
    
    # n_ct_scans: numeric, 결측 → 0
    n_ct_scans = if_else(is.na(n_ct_scans), 0, as.numeric(n_ct_scans)),
    
    # ct_any_positive: numeric (기존 자동 판정), 결측 → fever_focus로 대체
    ct_any_positive = if_else(is.na(ct_any_positive), 
                              as.numeric(fever_focus), 
                              as.numeric(ct_any_positive)),
    
    # ct_findings_combined: character, 결측 → "No CT performed"
    ct_findings_combined = if_else(is.na(ct_findings_combined) | 
                                     str_trim(ct_findings_combined) == "",
                                   "No CT performed",
...
# 변환 결과 확인
cat("변환된 데이터 구조:\n")
str(ct_summary_dedup)
cat("\n")

#------------------------------------------------------------------------------
# 4. CT 분석 결과 요약
#------------------------------------------------------------------------------
cat("=== STEP 4: CT 분석 결과 요약 ===\n")

ct_analysis_summary <- tibble(
  Metric = c("Total CT records", 
             "Fever focus positive (n)", 
             "Fever focus positive (%)",
             "Disease_1 identified",
             "Disease_2 identified",
             "Disease_3 identified"),
  Value = c(
...
# 가장 흔한 질환 Top 5
cat("가장 흔한 질환 (disease_1) Top 5:\n")
disease_freq <- ct_summary_dedup %>%
  filter(disease_1 != "None") %>%
  count(disease_1, sort = TRUE) %>%
...
#------------------------------------------------------------------------------
# 5. Part 2 데이터 로드 및 통합 준비
#------------------------------------------------------------------------------
cat("=== STEP 5: Part 2 데이터 로드 ===\n")
...
cat(sprintf("✓ Base: %d명\n", nrow(base_dedup)))
cat(sprintf("✓ Nurse: %d건\n", nrow(nurse_dedup)))
cat(sprintf("✓ Fever Lab: %d건\n", nrow(fever_lab_wide_dedup)))
cat(sprintf("✓ Fever Vitals: %d건\n", nrow(fever_vitals_summary)))
cat(sprintf("✓ CT Summary: %d건\n\n", nrow(ct_summary_dedup)))

#------------------------------------------------------------------------------
# 6. Inclusion/Exclusion 기준 적용
#------------------------------------------------------------------------------
cat("=== STEP 6: Inclusion/Exclusion 기준 ===\n")
...
# CT 변수 결측 처리는 이미 ct_summary_dedup에서 완료됨
...
cat("✓ base_analysis.rds (⭐ CT 분석 + 활력징후 포함!)\n")
...
ct_positive_summary <- base_ct %>%
  summarize(
    ...
    Auto_CT_Positive = sum(ct_any_positive == 1, na.rm = TRUE),
    Auto_CT_Pct = round(sum(ct_any_positive == 1, na.rm = TRUE) / n() * 100, 1)
  )
